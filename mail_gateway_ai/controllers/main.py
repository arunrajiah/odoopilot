...
class OdooPilotController(http.Controller):
    ...
    def telegram_webhook(self, **kwargs):
        ...
        secret = cfg.get_param("mail_gateway_ai.telegram_webhook_secret")
        if secret:
        ...
        enabled = cfg.get_param("mail_gateway_ai.telegram_enabled")
        if not enabled:
    ...
    def _dispatch_update(self, env, update):
        ...
        token = cfg.get_param("mail_gateway_ai.telegram_bot_token")
        if not token:
            return
        ...
        identity = env["mail.gateway.ai.identity"].search(
    ...
    def _handle_confirmation(self, env, tg, chat_id, payload):
        ...
        if payload == "confirm:no":
            ...
            session = env["mail.gateway.ai.session"].search(
    ...
    def link_start(self, token=None, **kwargs):
        ...
        if not token:
            return request.render("mail_gateway_ai.link_error", {"error": "Missing token."})

        cfg = request.env["ir.config_parameter"].sudo()
        raw = cfg.get_param(f"mail_gateway_ai.link_token.{token}")
        if not raw:
            return request.render("mail_gateway_ai.link_error", {"error": "Invalid or expired link."})

        try:
            data = json.loads(raw)
        except Exception:
            return request.render("mail_gateway_ai.link_error", {"error": "Corrupt token."})

        if int(time.time()) > data.get("exp", 0):
            cfg.set_param(f"mail_gateway_ai.link_token.{token}", "")
            return request.render("mail_gateway_ai.link_error", {"error": "This link has expired. Use /link again."})

        channel = data.get("channel", "telegram")
        chat_id = data.get("chat_id", "")

        # Check if already linked
        existing = request.env["mail.gateway.ai.identity"].search(
        ...
        if existing:
            ...
            request.env["mail.gateway.ai.identity"].create({
        ...
        bot_token = cfg.get_param("mail_gateway_ai.telegram_bot_token")
        ...
        return request.render("mail_gateway_ai.link_success", {"user": request.env.user})
