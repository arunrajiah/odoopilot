...
class MailGatewayAISession(models.Model):
    ...
    def get_or_create(self, channel, chat_id):
        session = self.search([("channel", "=", channel), ("chat_id", "=", chat_id)], limit=1)
        if not session:
    ...
    def append_message(self, role, content):
        ...
        if len(msgs) > 40:
            msgs = msgs[-40:]
        self.write({"messages_json": json.dumps(msgs), "updated_at": fields.Datetime.now()})
